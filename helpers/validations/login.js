...
const validateLoginInput = (userInput) => {
  const errors = {};
  ...
  if (validator.isEmpty(data.email)) { errors.email = 'Email field cannot be empty'; }

  if (validator.isEmpty(data.password)) { errors.password = 'Password field cannot be empty'; }
